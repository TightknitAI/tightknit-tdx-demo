...
export default SlackFunction(
  ...
  async ({ inputs, client }) => {
    ...
    if (
      ...
      !conversationsRepliesResponse.ok ||
      !conversationsRepliesResponse.messages
    ) {
      const errorMsg = `Error retrieving message replies. Contact the app maintainers with the following information - (Error detail: message "${message_ts}" in channel ${channel_id})`;
    ...
    if (!queryResponse || !queryResponse.ok) {
      const errorMsg = `Error retrieving conversation of parent thread history in datastore. Contact the app maintainers with the following information - (Error detail: ${queryResponse.error})`;
      console.log(errorMsg);
      return { error: errorMsg };
    }

    if (!queryResponse.items || queryResponse.items.length === 0) {
      const errorMsg = `No active tracked support conversation found for Slack thread_ts "${thread_ts}. Contact the app maintainers with the following information - (Error detail: ${queryResponse.error})`;
      console.log(errorMsg);
      return { error: errorMsg };
    }
    ...
    if (!usersInfoResponse || !usersInfoResponse.ok) {
      const errorMsg = `Error retrieving user info for sender of reply. Contact the app maintainers with the following information - (Error detail: ${queryResponse.error})`;
      console.log(errorMsg);
      return { error: errorMsg };
    }
    const senderName = usersInfoResponse.user.real_name;
    const senderPhotoUrl =
      usersInfoResponse.user.profile.image_72 ||
      usersInfoResponse.user.image_192 ||
      usersInfoResponse.user.image_512 ||
